...
function ViewProfile() {
    ...
    const handleSq= async (e) => {
        if(choose==="approved")
        {
            ...
            e.preventDefault()
       const url = "http://localhost:3001/RNC/viewprofile"; //url to fetch details from permanent table
       ...
       axios.post(url,{name:name,branch:branch}).then((response) => {
           ...
           console.log(response.data)                //required ones
            ...
            e.preventDefault()
            const url = "http://localhost:3001/RNC/viewprofile"; //url to fetch details from rejected table
           // const { data: res } = await axios.post(url, {title : title})  // must be post 
            axios.post(url,{name:name,branch:branch}).then((response) => {
